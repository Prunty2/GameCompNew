...
The harbor opens on **Market**, with **Cargo** and **Dock Services** as secondary tabs. Market first shows one independently scrollable grid of every discovered species beneath a single **Fish market** heading; day, condition, exchange, availability, trend, habitat, access, and competing-harbor labels do not compete for attention in this catalogue. Each compact card follows the same fish-first hierarchy as the first-assignment route step: a large real fish icon, the fish name, and a pill-shaped local whole-fish price in place of the requested count. Cards have clean rounded edges, one border, a dark depth shadow, and a restrained upward bounce on hover. The bounce becomes static when reduced motion is enabled. Undiscovered fish do not appear as anonymous locked cards.
