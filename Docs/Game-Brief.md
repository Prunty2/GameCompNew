...
The harbor interface opens on Delivery and separates Cargo and Dock services into focused submenus. One compact dark translucent dock panel creates a clear hierarchy of location and shells, category tabs, selected content, and secondary navigation. These sections share the same left and right edges. On a fresh save, the first guided assignment introduces the complete game loop without exposing later dock systems. After onboarding, every submenu omits the repeated objective panel and harbor subtitle so its functional content begins directly below the tabs. Delivery makes the job ticket the immediate visual focus: its job name is centred, its shell payment is explicitly labelled Reward, and Catch, Freshness, and Deliver appear as three large equal stages separated by small gaps without connector arrows. Each stage uses a numbered circular marker, a short label, and the exact species, threshold, or destination value; the Catch stage uses the real species artwork, Freshness uses a dedicated fish-and-leaf pictogram, and Deliver uses a generated harbor beacon. Accepted jobs keep the same route rather than replacing Catch with an abstract Accepted step; sea-glass completion and amber current-state treatments show progress without relying on colour alone. The primary accept or delivery action spans the ticket width, while active travel guidance uses the same dark panel language rather than a light legacy callout. Cargo uses a ten-slot game inventory: three slots are unlocked on a fresh save, caught fish occupy slots with freshness and an accessible cream-and-orange bin control for releasing that catch, and the other seven use a dedicated transparent cream-and-orange padlock pictogram. The release control uses a 44-pixel target while keeping its artwork visually quiet in the slot corner. Hover and keyboard focus lift the bin lid, activation fades the catch from its slot, and a five-second status message offers Undo before the release becomes final. Reduced-motion settings make both state changes immediate. Selecting a locked slot gives it the shared restrained mechanical wobble, then opens Dock Services and focuses its tab; neutral borders and focus rings keep amber reserved for primary state. The same roughly half-degree hover/click response applies consistently to enabled buttons across the harbor, main, settings, controls, and pause menus, while reduced-motion settings suppress it. Dock Services presents one clearly separated full-width row per cargo, engine, lamp, line-depth, and permit option without an additional heading panel. Each row uses an icon plate, concise effect copy, a live level meter, and a shell-price purchase control calculated from the real balance model; Repair hull is not offered. Only one harbor function is visible at a time, the selected submenu keeps keyboard focus, and each submenu must fit supported desktop, portrait, and short-landscape viewports without page scrolling. Switching submenus leaves the title block, tab bar, and footer fixed while only the inner content changes. The new page uses a quick directional fade-and-slide that follows the Delivery → Cargo → Services tab order; reduced-motion settings make the swap immediate. Tabs, service types, shell costs, and help actions use the shared transparent pictogram atlas with short visible labels, while route-specific artwork is imported as dedicated assets. Repeated explanatory sentences are removed when the icon and local heading already communicate the action. The background remains recognisable beneath a darker overlay and light blur.
...
4. At five times the interaction distance, a compact hook-and-arc cue begins fading smoothly into view, becomes prominently visible by four times the interaction distance, reaches full visibility by three times the interaction distance, and stays fully visible through the approach. Travelling away reverses the same opacity curve so the cue fades out without popping. It stays anchored to the ground's world position instead of following the boat; once the boat enters interaction range, its accessible interaction label supplies the site name and any permit, line-depth, speed, or cargo restriction without putting persistent text over the lake.
...
The surface cue combines two information layers. The always-present full shoal uses restrained but readable contrast and moves slowly enough to remain discoverable with reduced motion enabled. Proximity clearly strengthens the silhouettes, adds a soft vertical lens of clearer water, and reveals subtle caustic and orbit trails. The hook cue fades continuously from transparent at five times the real interaction radius to prominent visibility at four times that radius, reaches full visibility by three times that radius, and stays fully visible through the approach. Travelling away reverses the same curve for a smooth fade-out, while the interaction itself remains limited to the real radius. High-contrast mode strengthens silhouette edges and the hook arc; all restriction details remain available as accessible text and focus feedback rather than colour alone.

Fish should have recognizable silhouettes and movement patterns so catches do not depend on color alone. Paths are deterministic but species-specific: smooth wandering replaces rapid depth jukes, bursts follow uneven rest cycles, and a fish decelerates through near-zero speed before its heading changes instead of instantly reversing. The nine resident species use habitat-specific four-frame swim sheets with real freshwater anatomy, subdued natural colour, and restrained natural-history gouache detail. Tail, rear-body, pectoral-fin, or dorsal-ribbon motion changes between authored frames according to the species' real propulsion strategy while the simulation owns the actual path. Propulsion animation quiets while a fish hovers or waits in ambush; reduced-motion mode holds the neutral frame without changing gameplay movement. Fish artwork uses a thin integrated dark keyline rather than a pale sticker border. During fishing, a compact unboxed specimen note shows only the requested fish sprite and its name. A thin, partially translucent silhouette-following outline and a small chevron identify only the matching fish in the water; the chevron preserves a non-color cue while the outline color communicates rarity: common warm ivory, uncommon sea-glass green, rare amber, and legendary restrained violet. High contrast may strengthen this target outline without changing non-target fish. Rarer fish can use harder-to-read rest-and-burst timing, hide deeper, or require improved fishing equipment. Reeling remains a short automatic payoff rather than a separate tension minigame.

The underwater presentation is assembled from independent layers. Each fishing site has its own full-screen environment painting, while fish, hook, line, target outline, a sparse four-float line boundary, and labels remain separate movable or procedural layers. There is no vertical depth meter; the boundary alone communicates the current line limit. Species movement changes the actual simulation path as well as the sprite pose: Bluegill scull with their pectoral fins and add short tail bursts; Yellow Perch cruise steadily in loose schools; Emerald Shiner use quick burst-and-coast schooling motion; Northern Pike wait nearly motionless before rare explosive strikes; Largemouth Bass alternate compact tail beats with glides; Bowfin hold the body steady while the long dorsal ribbon fin undulates; Lake Trout sustain efficient posterior-body cruising; Burbot move slowly near the bed with a gentle whole-body wave; and Lake Sturgeon keep the armored forebody steady during broad, slow tail sweeps. Reduced-motion mode removes secondary body flex while preserving gameplay-affecting movement. Sunward Shoal uses warm blue-green shallows, reeds, cream reflected light, and submerged timber; Mosswater Pool uses greener low-visibility water, mossy roots, and silt pockets; Outer Gloam uses cold violet water, rock shelves, sparse pale growth, and poor visibility. This keeps the same fishing interaction readable everywhere without making the three grounds feel like one repeated aquarium.
...
| Site | Habitat role | Resident species and rarity |
| --- | --- | --- |
| Sunward Shoal | Warm dockside reeds and open nearshore water | Bluegill (common), Yellow Perch (common), Emerald Shiner (uncommon) |
| Mosswater Pool | Warm vegetated river pool with timber and low-oxygen pockets | Northern Pike (uncommon), Largemouth Bass (uncommon), Bowfin (rare) |
| Outer Gloam | Cold rocky drop-off and connected deep-water bed | Lake Trout (rare), Burbot (rare), Lake Sturgeon (legendary) |

### Real-species research basis

The lake is treated as a fictional Great Lakes-style system connected to tributary rivers and harbors. NOAA and Great Lakes fish-community records place Yellow Perch, Emerald Shiner, Northern Pike, Largemouth Bass, Lake Trout, Burbot, and Lake Sturgeon in this regional species pool; Wisconsin Sea Grant also identifies Bluegill and Bowfin as Great Lakes fishes. Habitat assignment follows the documented split between harbor/embayment, nearshore, offshore, and tributary zones rather than placing every species in one generic dock school.

Movement profiles are abstractions of published locomotor strategies rather than literal tracking replays. Bluegill use pectoral-fin and body/caudal-fin gaits; Largemouth Bass use intermittent propulsive beats and glides; Northern Pike are vegetation-associated sit-and-wait predators with explosive strikes; Bowfin propel slow swimming with waves along the dorsal ribbon fin; and the remaining body/caudal-fin swimmers vary posterior-body participation, cruising speed, depth drift, and burst frequency by shape and habitat. Sources: [NOAA Lake Erie fish inventory](https://www.fisheries.noaa.gov/inport/item/67690/full-list), [Great Lakes Fishery Commission habitat zones](https://www.glfc.org/pubs/SpecialPubs/Sp03_1.pdf), [Wisconsin Sea Grant Great Lakes fishes](https://www.seagrant.wisc.edu/news/get-your-own-brand-new-fish-of-the-great-lakes-poster/), [USFWS Lake Sturgeon profile](https://www.fws.gov/species/lake-sturgeon-acipenser-fulvescens), [comparative fish locomotion study](https://pmc.ncbi.nlm.nih.gov/articles/PMC2981590/), [Bluegill pectoral-fin kinematics](https://doi.org/10.1242/jeb.189.1.133), [Largemouth Bass intermittent swimming](https://pmc.ncbi.nlm.nih.gov/articles/PMC9065977/), [Northern Pike ambush behavior](https://pmc.ncbi.nlm.nih.gov/articles/PMC8410764/), and [Bowfin ribbon-fin kinematics](https://doi.org/10.1002/jez.1819).
...
Save data is versioned, validated, clamped, and migrated. Version 9 maps all nine retired fantasy species identifiers to their real-species replacements so existing discovery progress survives the rename. CrazyGames data storage is used when available, with local storage as a development fallback. Temporary simulation details should not be saved if restoring them could produce an invalid or unfair game state.
...
1. Start at **Brindle Harbor** and accept **The Morning Order**, requesting one Bluegill for **Gloam Ferry**.
