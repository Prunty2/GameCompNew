...
6. Compare survey and express routes from the catch site to the destination using distance, estimated time, and predicted freshness.
7. Cross the open lake through darkness and fog without fixed collision obstacles.
...
- **Immediate:** selected-answer state, written explanation, catch feedback, and objective guidance.
...
- **Decomposition:** travel, survey, fishing, population, contract, weather, rendering, input, persistence, and platform integration are separate responsibilities.
...
The harbor interface opens on Delivery and separates Cargo and Dock services into focused submenus. One compact dark translucent dock panel creates a five-part vertical hierarchy: location and shells, current objective, equal-width category tabs, selected content, and secondary navigation. These sections share the same left and right edges. On a fresh save, the objective welcomes the player with a compact Survey → Catch → Deliver primer before presenting the first guided assignment; this introduces the complete game loop without exposing later dock systems. The compact wordmark sits beside the harbor name, objective copy is left-aligned, the job ticket is the visual focus, and the lake action remains a neutral exit while amber is reserved for the selected tab, filled upgrade segments, reward, and primary job action. Dock Services centres the “Improve your boat” heading without a redundant “Dock services” eyebrow, then presents one clearly separated full-width row per cargo, engine, lamp, line-depth, and permit option. Each row uses an icon plate, concise effect copy, a six-segment live level meter, and a shell-price purchase control calculated from the real balance model; Repair hull is not offered. Only one harbor function is visible at a time, the selected submenu keeps keyboard focus, and each submenu must fit supported desktop, portrait, and short-landscape viewports without page scrolling. Switching submenus leaves the title block, objective, tab bar, and footer fixed while only the inner content changes. Tabs, service types, shell costs, help, and field-guide actions use the shared transparent pictogram atlas with short visible labels; each pictogram is centred within an equal sprite cell. Repeated explanatory sentences are removed when the icon and local heading already communicate the action. The background remains recognisable beneath a darker overlay and light blur.
...
- Route mathematics with survey/express time and freshness comparisons
...
- Survey/express route estimates and speed multipliers
...
- Do freshness and travel time create understandable route decisions?
...
6. Watch freshness and population change while crossing the open lake.
...
| Survey/express speed multiplier | 0.92 / 1.12 |
