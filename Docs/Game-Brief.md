...
The harbor opens on **Market**, with **Cargo** and **Dock Services** as secondary tabs. Market first shows one independently scrollable grid of every discovered species. Each compact card follows the same fish-first hierarchy as the first-assignment route step: a large real fish icon, the fish name, and a pill-shaped local whole-fish price in place of the requested count. Availability, trend, habitat, access, and competing-harbor facts do not compete for attention in this catalogue. Cards use a restrained wobble response before opening and become static when reduced motion is enabled. Undiscovered fish do not appear as anonymous locked cards.

Selecting a fish replaces the catalogue with a focused detail view instead of keeping both views visible. The left side contains a large fish icon, name, current local price, cargo quantity, a compact Track pill, the primary Sell action, and a clear Back to market action. The right side is dominated by the seven-day labelled price graph. The graph uses points, axes, numbers, and trend text so price movement is never communicated by colour alone. On narrow screens the fish summary stacks above the graph in one natural vertical scroller. Buttons retain 44-pixel touch targets. Warm amber is reserved for the selected listing, primary sale or tracking action, and the current tutorial target. The rest of the board uses dark teal, warm ivory, thin rules, and the existing illustrated dock materials. No glass panels, decorative gradients, ornamental glow, or dashboard-style fact grids are introduced.
