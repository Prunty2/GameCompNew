...
After eight market sales, the player receives a season report summarising earnings, species discoveries, and trading activity. The loop remains playable afterward, while discovering all nine species, unlocking every region, and purchasing every upgrade provides longer-term mastery.
...
The harbor opens on **Market**, with **Cargo** and **Dock Services** as secondary tabs. Market uses a two-column dock ledger on wide screens and a stacked layout on narrow screens. Its left side is an independently scrollable grid of every discovered species, each showing the real fish icon, name, local whole-fish quote, and a labelled rising, steady, or falling trend. Selecting a listing opens a larger detail view containing the species icon, current local quote, the other harbor's quote, a seven-day labelled line graph, habitat, fishing ground, depth requirement, cargo count, freshness-adjusted sale total, Track action, and Sell action. The graph uses points, axes, numbers, and trend text so price movement is never communicated by colour alone. The selected detail remains visible while the listing grid scrolls. Undiscovered fish do not appear as anonymous locked cards.

Only the market list may scroll inside the fixed harbor shell on desktop. On short landscape and portrait screens, the market content becomes one natural vertical scroller, with a sticky species selector and sale action where space permits. Buttons retain 44-pixel touch targets. Warm amber is reserved for the selected listing, primary sale or tracking action, and the current tutorial target. The rest of the board uses dark teal, warm ivory, thin rules, and the existing illustrated dock materials. No glass panels, decorative gradients, floating dashboard cards, or ornamental glow are introduced.
...
The current lake condition is named on the market board. Conditions such as warm shallows, cold water, clear weather, or fog affect related habitat groups by a small documented amount. Conditions also affect real travel and species availability through existing weather, region, depth, and permit systems. The model must not claim that a condition changes a catch if the simulation does not enforce that relationship.

The sale value of each catch is its local whole-fish quote multiplied by a freshness factor. At 100% freshness the fish receives the full quote. Above 0% freshness the factor scales from 25% to 100%; a spoiled fish cannot be sold. Selling a species sells all non-spoiled catches of that species in one transaction, removes only those catches, and shows quantity, average freshness, quoted value, freshness adjustment, and total shells earned before or immediately after confirmation.
...
- **Species availability:** only discovered fish appear on the board, and habitat, resident set, depth, permit, and current lake condition determine where they can be caught.
...
- Use HTML/CSS overlays for the market, shops, dialogue, settings, tutorials, and touch controls.
