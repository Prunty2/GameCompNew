...
9. The fish follows the rising hook on a taut line with a restrained struggle while the camera eases back toward the sailing waterline, reaching the surface framing before the fish enters cargo. Reduced-motion mode removes the struggle and shows the final framing immediately while keeping the hooked and secured states readable.
