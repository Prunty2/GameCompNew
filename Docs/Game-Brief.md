...
The game deliberately places the question before the answer. A survey asks the player to make a prediction, records it, immediately explains the best-supported species, and then lets the player test that evidence underwater. The season report retains aggregate results so learning is visible over time.
...
The lake panorama and every surface effect share one normalized world-space camera projection. The camera owns a single visible span, clamped left edge, speed-based look-ahead, and restrained damped follow slack; background crop bounds, harbors, fishing-ground shoals, objectives, weather lighting, and the boat all derive their screen positions from that same projection. This prevents scenery and gameplay objects from drifting at different apparent speeds while letting acceleration and braking move the boat slightly within the frame before the view settles. Reduced-motion mode uses the direct camera, and the title retains its wider static view.
...
- Large, soft white steam clouds rise from the tugboat stack and stretch into a longer trail with speed; at full speed, seeded downwash and turbulence keep the plume lower and loosely animated. Eight painted sprite variations rotate deterministically so the plume stays organic without reading as dark pollution smoke
...
- End-of-season mastery report with prediction and lake-health results
...
- View prediction-versus-result feedback and the season report
