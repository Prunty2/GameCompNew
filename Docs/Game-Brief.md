...
The Fishing interaction owns its temporary steering, hooked-reel, and empty-exit phases plus their deterministic timing and read-only presentation projection. Cargo, delivery contracts, and Canvas drawing remain outside that module, and active Fishing interaction state is not persisted.
