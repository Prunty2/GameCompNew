...
A site spawns every resident. Count per resident follows that day's availability: 3 abundant, 2 normal, 1 scarce. Hook contact within radius 0.058 on a reachable depth starts a deterministic fight. The fish cycles through calm lulls, sprints away from the boat, and short thrashes, darting on the line instead of sliding out and snapping back. Holding left click, touch, or the Reel key during a lull gains ground; horsing a run loads the line. Releasing during a run lets the fish race away, which is what drops tension. Tension is read from the hook line colour and thickness rather than a corner meter. Non-hooked fish freeze, fade, and lose their targeting cues during the fight. Rarity scales run length and pull strength. Sustained critical tension breaks the line without ending the fishing session, while a completed fight uses a 1.15 s landing transition and stores the catch at 100% freshness.
