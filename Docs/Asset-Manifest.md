...
- Runtime role: harbor- and time-specific full-screen settings behind the shared dock menu. Reedbank Harbor is a practical working pier; Beacon Point is a colder, isolated outer-lake landing. Each harbor's day and night plates blend using the sailing scene's night intensity. The paintings are interface backdrops only and contain no boat, characters, labels, or UI; readable harbor-name signs are drawn as separate world-space labels.
