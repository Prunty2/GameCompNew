...
- Generation mode: OpenAI built-in image generation using `lake-chart.png` as the authoritative painting, palette, atmosphere, and world-scale reference, `harbor-pier.png` for timber and narrow-plank construction, and `player-boat.png` for scale only. Each nighttime plate is a lighting-weather edit of its approved daytime anchor and preserves the anchor's exact camera, geometry, and placement.
- Shared prompt direction: a medium-wide, zoomed-out dock environment in restrained editorial gouache and screen print. The pier is a slim human-scale structure with roughly 36–44 narrow plank divisions; harbor identity stays in an outer quarter around a calm low-detail center behind the menu panel. Full bleed; no boat, fish, people, text, logo, UI, detached sprite, chroma key, or transparency.
- Brindle direction: practical freshwater working harbor attached to the left shoreline, with small sheds, creels, rope, reeds, and low wooded hills confined to the outer left quarter. Its pier extends rightward across open water to a free terminal end on the right. The day plate uses warm overcast light; its composition-matched night edit uses readable blue-black color and restrained amber work lamps.
- Gloam direction: cold outer-lake ferry landing attached to the right shoreline, with a small ferry shelter, rock shelf, sparse pale reeds, and dark conifers confined to the outer right quarter. Its pier extends leftward across open water to a clearly visible free terminal and beacon on the left, matching the player's approach from that direction. The day plate uses cold clear overcast light; its composition-matched night edit uses deep violet-blue color and localized amber beacon light.
