...
async function musicState(
  ...
  const audio = page.locator(`audio[data-music-scene="${scene}"]`);
...
async function menuMusicState(page: import("@playwright/test").Page) {
  return musicState(page, "menu");
}

async function gameMusicState(page: import("@playwright/test").Page) {
  return musicState(page, "game");
}
...
test("scene music loops and crossfades between the menu and gameplay", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator('audio[data-music-scene="menu"]')).toHaveCount(1);
  await expect(page.locator('audio[data-music-scene="game"]')).toHaveCount(1);
  await expect.poll(() => menuMusicState(page)).toEqual(expect.objectContaining({ loop: true }));
  ...
  await expect.poll(() => gameMusicState(page)).toEqual(expect.objectContaining({ paused: false, muted: false }));
  ...
  await expect.poll(() => menuMusicState(page)).toEqual(expect.objectContaining({ loop: true, paused: false, muted: false }));
  await expect.poll(() => gameMusicState(page)).toEqual(expect.objectContaining({ paused: true, volume: 0, currentTime: 0 }));
});

test("game music continues when travelling to the Beach", async ({ page }) => {
  await page.goto("/?e2e=1");
  await page.evaluate(() => {
    window.localStorage.setItem("gamecomp-new.save", JSON.stringify({
      version: 11,
      progress: { money: 1_000, beachUnlocked: true },
      settings: {},
    }));
  });
  await page.reload();
...
test("mute and music volume control game music independently of sound effects", async ({ page }) => {
  ...
  await mute.click();
  await expect.poll(() => menuMusicState(page)).toEqual(expect.objectContaining({ loop: true, paused: true, muted: true, volume: 0 }));
...
test("credits lists the team and returns to the main menu", async ({ page }) => {
  ...
  await expect(page.getByText("Lead Programmer / Game Designer / Gameplay Tester")).toBeVisible();
  await expect(page.getByText("Programmer / Visual Designer / Gameplay Tester")).toBeVisible();
  await expect(page.getByText("Progression Writer / Documentation / Gameplay Tester")).toBeVisible();
  await expect(page.getByText("Sound Designer / Marine Specialist")).toBeVisible();
...
test("hooked fish expose working reel and release controls", async ({ page }) => {
  ...
  const reelResponse = await canvas.evaluate((element) => {
    const rendered = element as HTMLCanvasElement;
    window.__FSHING_TEST__?.primeFishingRun(0.54);
    const starting = {
      progress: Number(rendered.dataset.fishingReelProgress),
      tension: Number(rendered.dataset.fishingLineTension),
      colour: rendered.dataset.fishingLineColour ?? null,
      backgroundPoseTime: Number(rendered.dataset.fishingBackgroundPoseElapsed),
    };
    rendered.dispatchEvent(new PointerEvent("pointerdown", {
      pointerId: 1,
      button: 0,
      isPrimary: true,
      bubbles: true,
    }));
    window.__FSHING_TEST__?.stepFishing(12);
    const pulled = {
      progress: Number(rendered.dataset.fishingReelProgress),
      tension: Number(rendered.dataset.fishingLineTension),
      colour: rendered.dataset.fishingLineColour ?? null,
    ...
    return {
      starting,
      pulled,
      restedTension: Number(rendered.dataset.fishingLineTension),
    };
  });
  expect(reelResponse.pulled.progress).toBeGreaterThan(reelResponse.starting.progress);
  expect(reelResponse.pulled.tension).toBeGreaterThan(reelResponse.starting.tension);
  expect(reelResponse.pulled.colour).not.toBe(reelResponse.starting.colour);
  expect(reelResponse.pulled.backgroundPoseTime).toBeGreaterThan(reelResponse.starting.backgroundPoseTime);
  expect(reelResponse.restedTension).toBeLessThan(reelResponse.pulled.tension);
...
test("a snapped line lets the fish escape before retracting without a hook", async ({ page }) => {
  ...
  const canvas = page.locator("#game-canvas");
  await expect(canvas).toHaveAttribute("data-fishing-state", "fighting");
  const lineSnap = await canvas.evaluate((element) => {
    window.__FSHING_TEST__?.snapLine();
    const toast = document.querySelector<HTMLElement>("#toast");
    const tutorial = document.querySelector<HTMLElement>("#market-tutorial");
    const toastBox = toast?.getBoundingClientRect();
    const tutorialBox = tutorial?.getBoundingClientRect();
    const feedback = {
      className: toast?.className ?? "",
      text: toast?.textContent ?? "",
      iconText: toast?.querySelector<HTMLElement>(".toast-danger-icon")?.textContent ?? "",
      toastBox: toastBox ? {
        x: toastBox.x,
        y: toastBox.y,
    ...
    for (let index = 0; index < 600; index += 1) {
      window.__FSHING_TEST__?.stepFishing(1);
      const rendered = element as HTMLCanvasElement;
      samples.push({
      ...
      if (rendered.dataset.fishingState === "steering"
        && samples.some((sample) => sample.phase === "line-retracting")) {
        return { feedback, samples };
      }
    }
    throw new Error("Fishing session did not return to steering after line retraction.");
  });

  const { feedback, samples: phaseSamples } = lineSnap;
  expect(feedback.className).toContain("is-line-snap");
  expect(feedback.text).toContain("The line snapped!");
  expect(feedback.iconText).toBe("×");
  const { toastBox, tutorialBox } = feedback;
  expect(toastBox).not.toBeNull();
  expect(tutorialBox).not.toBeNull();
  expect(Math.abs((toastBox!.x + toastBox!.width / 2) - feedback.viewportWidth / 2)).toBeLessThan(2);
...
test("settings reset save asks for confirmation before restoring the first assignment", async ({ page }) => {
  ...
  await expect(page.getByText("Reset save?")).toBeVisible();
