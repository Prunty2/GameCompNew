...
describe("versioned save data", () => {
  ...
  test("migrates the starter version without carrying theme-neutral score", () => {
    ...
    expect(loadSave(storage, true).settings.fullscreen).toBe(true);
  ...
  test("migrates older desktop saves to fullscreen and then preserves the preference", () => {
    const baseline = defaultSave();
    const legacyStorage = memoryStorage(JSON.stringify({
      version: 15,
      progress: baseline.progress,
      settings: { ...baseline.settings, fullscreen: false },
    }));
    expect(loadSave(legacyStorage, true).settings.fullscreen).toBe(true);

    const currentStorage = memoryStorage(JSON.stringify({
      version: 16,
      progress: baseline.progress,
      settings: { ...baseline.settings, fullscreen: false },
    }));
    expect(loadSave(currentStorage, true).settings.fullscreen).toBe(false);
  });
