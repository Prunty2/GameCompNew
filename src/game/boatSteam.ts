...
export function boatSteamPuffs(
  ...
  for (let index = 0; index < PUFF_COUNT; index += 1) {
    ...
    const tone = seededUnit(seed + 18.9) * 2 - 1;
    ...
    const wander = Math.sin(age * Math.PI * (1.2 + flattening * 0.7) + tone * Math.PI)
    ...
    puffs.push({
      ...
      stretchX: 1 + easedAge * (0.35 + flattening * 0.4),
      stretchY: 1 - easedAge * (0.16 + flattening * 0.2),
